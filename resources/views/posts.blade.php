...
@if ($posts[0]->image)
        ...
        @else
        <img src="https://source.unsplash.com/random/500x400?{{  $posts[0]->category->name }}" 
        class="card-img-top" alt="{{ $posts[0]->category->name  }}" class="img-fluid">
        
        @endif

  <div class="card-body text-center">
    <h3 class="card-title"><a href="{{url('/posts/'. $posts[0]->slug)}}"class="text-decoration-none text-dark"> {{$posts[0]->title}}</h3>
    <p>
    <small class="text-muted">
     By. <a href="{{url('/posts?author')}}={{$posts[0]->author->username}}"class="text-decoration-none">{{ $posts[0]->author->name }}</a> in<a href="{{url('/posts?category')}}={{ $posts[0]->category->slug }}"
...
<div class="container">
    ...
    <div class="card">
        <div class="position-absolute px-3 py-2 "style="background-color: rgba(0,0,0,0.7)"><a href="{{url('posts?category')}}={{ $post->category->slug }}" class="text-white text-decoration-none">{{
